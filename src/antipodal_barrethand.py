...
class AntiPodalBarretGrasping:
    def __init__(self, robot_id, body_indices, arm_indices, gripper_indices, max_width=0.1, table_height=0.62):
        ...
        self.body_indices = body_indices       # e.g., [0, 1, 2, 3, 4, 5]
        self.arm_indices = arm_indices         # e.g., [6, 7, 8, 9, 10, 11, 12]
        self.gripper_indices = gripper_indices # e.g., [13, 14, 15, 16, 17, 18, 19, 20]
        self.base_index = 0
        ...
        for i in range(p.getNumJoints(robot_id)):
            ...
            # CHANGE THIS: Look for your new link name
            if link_name == "virtual_grasp_frame": 
                self.ee_index = i
                print(f"Found Virtual End Effector at Index: {self.ee_index}")
                break
    ...
    def _solve_ik(self, target_pos, target_orn, current_joint_angles):
        ...
        joint_poses = p.calculateInverseKinematics(
            ...
            jointDamping=[0.01]*21, # Optional: Helps stability
    ...
    def get_action(self, obs):
        ...
        # 1. LOCK SPREAD TO 0 (Parallel Mode)
        # This forces Finger 2 and 3 to sit next to each other, acting as one "Virtual Finger"
        action[idx_spread] = 0.0
        action[18] = 0.0
        action[idx_f1] = gripper_cmd
        action[idx_f2] = gripper_cmd
        action[idx_f3] = gripper_cmd
        ...
        if self.state == "IDLE":
            ...
            # If target grasp is too low
            if (self.target_grasp[0][2] < obs['object_pos'][2]):
